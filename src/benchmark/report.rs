...
impl Report {
    ...
    pub fn write(&self) -> io::Result<()> {
        ...
        writeln!(
            summary,
            "version,mode,per_team,total_units,workload,repeat,samples,mean_update_ms,p50_update_ms,p95_update_ms,p99_update_ms,max_update_ms,mean_frame_ms,p95_frame_ms,order_ms,planning_total_ms,planned,failed,arrived,correctness_pass,timing_valid,checksum"
        )?;
        writeln!(
            samples,
            "mode,per_team,workload,repeat,tick,update_ms,frame_ms,planning_ms,moving,pending"
        )?;
        writeln!(
            markdown,
            "# RTS v{} benchmark\n\nProfile: **{}** · OS: {} · architecture: {}\n\nHeadless `update_ms` measures `App::update` CPU wall time at a fixed simulation step of 1/60 s. Graphical `update_ms` measures the main schedule only; `frame_ms` includes rendering/presentation waits. Headless values are not graphical FPS.\n\n| Mode | Units/team | Workload | Repeat | Mean CPU ms | p95 CPU ms | p99 CPU ms | Max CPU ms | Arrived | Correct | Timing valid |\n|---|---:|---|---:|---:|---:|---:|---:|---:|---|---|",
